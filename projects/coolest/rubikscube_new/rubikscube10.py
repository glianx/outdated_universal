...
def bfs(stage,prev_state=[],i=None,):
    ...
    if stage in ['cross','pair','slot']: 
        ...
        oll_data = open('test5.txt').read()
        oll_data = oll_data.split('\n')
        for i in range(len(oll_data)):
            oll_data[i] = oll_data[i].split()
        
        algs = [oll_data,None]

        for _urot in range(4):
            for state in algs[['oll','pll'].index(stage)]:
