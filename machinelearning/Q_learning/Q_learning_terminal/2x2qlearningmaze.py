...
def main():
  ...
  x = -0.1
  #'''
  F = np.zeros(shape=[4,4], dtype=np.int)  # Feasible 
  F[0,1] = 1; F[1,0] = 1
  F[0,2] = 1; F[2,0] = 1
  F[1,3] = 1; F[3,3] = 1
  

  R = np.zeros(shape=[4,4], dtype=np.int)  # Rewards
  R[0,1] = x; R[1,0] = x
  R[0,2] = x; R[2,0] = x
  R[1,3] = 10; R[3,3] = x
  #'''
...
# =============================================================
  ...
  Q = np.zeros(shape=[4,4], dtype=np.float32)  # Quality
  
  print("Analyzing maze with RL Q-learning")
  start = 0; goal = 3 #start and end
